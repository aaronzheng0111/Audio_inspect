...
def _build_pdf(
    ...
    *,
    ...
    # ── Font ─────────────────────────────────────────────────────────────
    # Helvetica is the primary font — proper Latin letter-spacing for European text.
    # STSong-Light is registered as a secondary CID font only when CJK characters
    # are actually present in the data; for pure Latin datasets it is never used.
    cjk = "Helvetica"
    bold = "Helvetica-Bold"
    _cjk_count = int(
        df["sentence"]
        .astype(str)
        .str.contains(r"[\u4e00-\u9fff\u3040-\u30ff]", regex=True)
        .sum()
    )
    # Only switch to CJK font when CJK sentences make up > 1 % of the dataset.
    # A handful of incidental CJK characters in a Latin corpus (e.g. 13 out of
    # 950 000 German sentences discussing Chinese radicals) must NOT force the
    # entire document into STSong-Light, which renders Latin with very tight
    # letter-spacing.
    _has_cjk = _cjk_count / max(len(df), 1) > 0.01
    if _has_cjk:
        try:
    ...
    if n_high:
        ...
        for i, r in enumerate(out_high.head(n_high).itertuples(index=False), 1):
            ...
            tag_data = [[
                ...
                Paragraph(_escape_xml(str(r.path)), sTagBlue),
            ...
            tag_tbl = Table(tag_data, colWidths=[0.6*cm, 8.8*cm, 2.2*cm, 2.0*cm, 2.8*cm])
            ...
            story.append(KeepTogether([tag_tbl, trans_tbl, Spacer(1, 10)]))
    ...
    if n_fast:
        ...
        for i, r in enumerate(out_fast.head(n_fast).itertuples(index=False), 1):
            ...
            tag_data = [[
                ...
                Paragraph(_escape_xml(str(r.path)), sTagAmber),
...
def _build_markdown(
    ...
    *,
    ...
    for i, r in enumerate(out_high.head(md_fulltext_n).itertuples(index=False), 1):
        lines += [
            f"### {i}. `{r.path}`",
            f"- duration_s={float(r.duration_s):.3f}  char_len={int(r.char_len)}"
...
def build_report(
    ...
    *,
    ...
    if "client_id" in df.columns:
        extra_stats["n_unique_speakers"] = int(df["client_id"].nunique())
