...
ALIASES: Dict[str, List[str]] = {
    "audio_name_id": ["audio_name_id", "id", "name", "utt_id", "utterance_id", "filename"],
    "text": ["text", "transcript", "transcription", "sentence", "label", "content"],
    "audio_path": ["audio_path", "path", "wav", "wav_path", "file", "filepath", "audio"],
}
...
class AttributeMapper:
    ...
    def validate(self, mapping: Dict[str, str]) -> None:
        ...
        for canonical, source in mapping.items():
            if source and source not in self.columns:
                raise ValueError(
                    f"Mapped column {source!r} for {canonical!r} does not exist."
                )
        if not mapping.get("audio_path"):
            raise ValueError(
